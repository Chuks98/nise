...
$(() => {
  if (window.location.pathname === '/student-registration') {
    ...
    $('#password').on('input', function () {
        ...
        if (password.length < 6) {
            ...
            feedback.text("Strong password ✔").css("color", "green");
        }
    });
  ...
  if(window.location.pathname === '/student-dashboard/check-result' || window.location.pathname === '/student-dashboard'){
    $('#resultCheckForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Inject result data into the right side
                $('#resultDisplay').html(`
                    <div class="p-2">
                        ...
                        <h6>Subjects & Scores</h6>
                        ${response.scores_html}
                    </div>
                `);
            },
  ...
  if(window.location.pathname === '/dashboard/blog') {
    ...
    function renderPagination(current, total, callback) {
      ...
      const createButton = (pageNum, isActive = false) => {
        ...
        btn.on('click', () => callback(pageNum)); // 👈 Use callback
        return btn;
      };
...
$(() => {
  // 🌟 GLOBAL AJAX LOADING INDICATOR + AUTO RESPONSE ALERT 🌟
  ...
  // -----------------------------
  // 4️⃣ PAGINATION RENDER (Reusable)
  // -----------------------------
  function renderPagination(current, total, callback) {
      ...
      // Next
      if (current < total) container.append(createBtn(current + 1, false, 'Next »'));
  }


  // -----------------------------
  // 5️⃣ FETCH CSV RECORDS (OLD SYSTEM)
  // -----------------------------
  ...
  // -----------------------------
  // 6️⃣ EVENT BINDINGS (ONLY ON UPLOAD PAGE)
  // -----------------------------
  if (isUploadPage) {
    ...
    if (window.location.pathname === '/dashboard/list') {
      ...
      function fetchScores(page = 1) {
        ...
        $.get('/scores/list', filters, function(res) {
          ...
          // ✅ Close loading Swal when done
          Swal.close();
      })
    }

      // Filters — ALWAYS reload page 1
  ...
  $(function() {
    ...
    if(window.location.pathname === '/dashboard/change-password') {
      ...
      $('#changePasswordForm').on('submit', function(e) {
        ...
        // 🚨 VALIDATION: must be 6+ chars, contain letters + numbers
        const isValidPassword = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,}$/;
        ...
        // If validation passed → continue with AJAX
        $.ajax({
